...
The ROC curve plot for decease status (multivariate model) is shown below and AUROC was 0.808 on training data and 0.805 on 10-fold cross-validation respectively. 
...
```{r eval = TRUE}
    ...
    legend("bottomright", legend = c("GLM Training", "GLM Cross-Validation"), col = c("black", "darkblue"), lwd = 2)
    
    auc(smoking_set$deceased.status, glm.top.pred)
    auc(obs_outputs, pred_outputs.glm)
```
The ROC curve plot for ventilator use (multivariate model) is shown below and AUROC was 0.652 on training data and 0.653 on 10-fold cross-validation respectively. 
...
```{r eval = TRUE}
      ...
      #ROC Curves
      library(pROC)
      plot.roc(smoking_set$ventilator, glm.top.pred, ci = TRUE, main = "Ventilator Outcome") 
      plot.roc(obs_outputs, pred_outputs.glm, ci = TRUE, col = "darkblue", add = TRUE) 
      legend("bottomright", legend = c("GLM Training", "GLM Cross-Validation"), col = c("black", "darkblue"), lwd = 2)
      ...
      auc(obs_outputs, pred_outputs.glm)
```
The ROC curve plot for ICU admission (multivariate model) is shown below and AUROC was 0.625 on training data and 0.638 on 10-fold cross-validation respectively. 
...
```{r eval = TRUE}     
      ...
      #ROC Curves
      library(pROC)
      plot.roc(smoking_set$ventilator, glm.top.pred, ci = TRUE, main = "ICU Outcome") 
      plot.roc(obs_outputs, pred_outputs.glm, ci = TRUE, col = "darkblue", add = TRUE) 
      legend("bottomright", legend = c("GLM Training", "GLM Cross-Validation"), col = c("black", "darkblue"), lwd = 2)
      ...
      auc(obs_outputs, pred_outputs.glm)
``` 
Conclusion: Overall, this project has provided some meaningful insights showing a relationship between patient characterstics and the COVID outcomes. The univariate analysis revealed that for ICU admission,ventilator use and mortality, the varaibles such as age, gender, smoking history and obesity have an association to predcict the outcomes. 
