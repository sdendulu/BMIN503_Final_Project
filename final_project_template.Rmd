...
#Clean Data Set
#COVID.clean1 <- Pat_Merged_Data %>% 
 #dplyr:: select(Random_Number,LOS, Ventilator_YN, ICU_Dept_YN, Deceased_Status, Category,
               # Age, Gender, Race, Smoking_Status,HTN,Diabetes,Angina,Hyperlipidemia,Stroke,COPD,CKD,Obesity,Asthma) %>%
 #filter(!Race  %in% c("Other","Unknown","Patient Declined")) %>% 
  # filter(Smoking_Status != 'Not Asked') 
...
unique(COVID.clean1$Gender)

## group by Gender
Pat_counts <- COVID.clean1%>% group_by(Gender) %>%
  summarize(count = n())
...
ggplot(data = Cat_counts, aes(x = Category, y = count, fill = Race)) +
  ggtitle("Confirmed and Ruled-out Patients and Race") +
  geom_bar(stat = 'identity')

 ggplot(data = Cat_counts, aes(x = Category, y = count, fill = Deceased_Status)) +
  ggtitle("Confirmed and Ruled-out Patients and Deceased Status") +
  geom_bar(stat = "identity")

ggplot(data = Cat_counts2, aes(x = Race, y = count, fill = Category)) +
  ggtitle("Patients Race and COVID Status") +
  geom_bar(stat = 'identity')

ggplot(data = Cat_counts2, aes(x = Race, y = count, fill = Deceased_Status)) + 
  geom_bar(stat = 'identity')



##mean average length of stay based on race
Avg_LOS <-COVID.clean1  %>% 
...
demo.glm1 <- glm(as.numeric(LOS) ~ as.numeric(Age) + as.numeric(Gender) + as.numeric(Race) + as.numeric(Smoking_Status), data = COVID.clean1)
summary(demo.glm1)

demo.glm2 <- glm(as.numeric(ICU_Dept_YN) ~ as.numeric(Age) + as.numeric(Gender) + as.numeric(Race) + as.numeric(Smoking_Status) + as.numeric(Obesity), data = COVID.clean1)
summary(demo.glm2)

demo.lm2 <- lm(as.numeric(ICU_Dept_YN) ~ as.numeric(LOS), data = COVID.clean1)
summary(demo.lm2)

demo.glm3 <- glm(as.numeric(Deceased_Status) ~ as.numeric(Age) + as.numeric(Gender) + as.numeric(Race) + as.numeric(Smoking_Status) + as.numeric(Obesity), data = COVID.clean1)
summary(demo.glm3)

demo.glm4 <- glm(as.numeric(Ventilator_YN) ~ as.numeric(Age) + as.numeric(Gender) + as.numeric(Race) + as.numeric(Smoking_Status), data = COVID.clean1)
summary(demo.glm4)



ggplot(COVID.clean1, aes(x = Age, y = ICU_Dept_YN)) +     # very significant
  geom_boxplot()

ggplot(COVID.clean1, aes(x = Gender, y = ICU_Dept_YN)) +  # very significant
  geom_bar(stat = 'identity')

ggplot(COVID.clean1, aes(x = Race, y = ICU_Dept_YN)) +  # significant
  geom_bar(stat = 'identity')

ggplot(COVID.clean1, aes(x = Smoking_Status, y = ICU_Dept_YN)) + 
  geom_bar(stat = 'identity')

ggplot(COVID.clean1, aes(x = Obesity, y = ICU_Dept_YN)) + # very significant
  geom_bar(stat = 'identity')

ggplot(COVID.clean1, aes(x = Ventilator_YN, y = ICU_Dept_YN)) + 
  geom_bar(stat = 'identity')

ggplot(COVID.clean1, aes(x = as.numeric(LOS), y = ICU_Dept_YN)) + # very significant
  geom_boxplot()
...
covid.hclust <- hclust(dist(COVID.clean1), method = "complete")
covid.dend <- dendro_data(as.dendrogram(covid.hclust))
labels <- label(covid.dend)
labels$Deceased_Status <- COVID.clean1$Deceased_Status[as.numeric(labels$label)]
