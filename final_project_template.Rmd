...
COVID.clean <- Pat_Merged_Data %>% 
        ...
        rename(age = Age,race = Race,gender= Gender,ethnicity = Ethnicity,smoking.status = Smoking_Status, 
               length.of.stay = LOS, icu=ICU_Dept_YN, ventilator = Ventilator_YN) %>%   
...
## group by Gender
Pat_counts <- COVID.clean%>% group_by(Gender) %>%
  summarize(count = n())
...
Cat_counts2 <- COVID.clean %>%
  filter(Category == 'Confirmed' | Category == 'Ruled Out') %>%
  filter(Race == 'White' | Race == 'Black') %>%
  group_by(Category,Race,Deceased_Status) %>%
  summarize(count = n())

Cat_counts3 <- COVID.clean %>%
  filter(Category == 'Confirmed' | Category == 'Ruled Out') %>%
  filter(Race == 'White' | Race == 'Black') %>%
  group_by(Category,Race,Deceased_Status) %>%
  summarize(count = n())
