...
but patient-level observations of ICU admission, ventilator requirement and even mortality by race are very limited. 
The objective of this study is to systematically determine what patient characteristics including race are associated 
...
are mostly likely to be affected by COVID-19 with higher infection rates, higher disease severity and even mortality. 
...
The de-identified data sets used for this project were pulled from the PennMedicine’s Clarity Database on patients 
...
Using SQL-queries, I obtained two data files. First I have pulled the patients who have had COVID-19 confirmed-postive 
...
```
In addition to Yes/No, the smoking status variable also contains information such as 'Not Asked", so a new data set was created of patients who answered Yes/No to evaulate the effects of smoking on the outcomes.
...
The contents of age variable is described as mean(sd)and for other variables, it was described as the percentage of 
the yes values of the Boolean variables (Male, Black, Obese, Tobacco exposure, ICU admission, Ventilator use 
...
```
In contrast, the chi-squre analysis on decease status against gender, race, obesity and smoking.status shows that only race and smoking status showed an association exist between the observed and expected as shown in the tables below.
...
The univariate anyalysis of each predictor variable against the each outcome variable has been shown in the following tables. For ICU against age, the p-values for intercept and age were less than 0.001 showing they were very significant. 
...
```
Unlike ICU, the univariate analysis for ventilator against each predictor variables, except race, all the other varaiables (age,gender, smoking status and obesity) were significant. The p-value for race was 0.372 though the intercept was highly significant (p<0.001). The univariate analysis data for ventilator against the each predictor were shown below.
...
auc(obs_outputs, pred_outputs.rf)
```
On the training data and on 10-fold cross-validation for logistic regression, the AOCs were 0.652 and 0.643 where for RF, the AOCs were 0.58 and 0.51 respectively and the curve plots were shown below.
...
Overall, this project has provided some meaningful insights showing a relationship between patient characterstics and the COVID outcomes. The chi-square analysis between each predictor variable and outcome varaible indicated that there was a significant difference between the expected and observed with a p<.005. The current findings on univariate analysis suggest that age and smoking status were independently associated with ICU, ventialtor use and mortality (decease status) outcomes. Among the multivariate logistic regression models, age, gender and obesity were the significant predictors for ICU admission and ventilator whereas age, race and smoking status were the significant predictors mortality (decease status) outcomes. For ICU admission and ventilator use outcomes, race and smoking status were not significant predcitors and for the mortality (decease status) multivariable regression model, gender and obesity were not significant predcitors. For all combinations of independent variables studied for ICU, ventilator use and decease status, the prediction probability for ICU admission was 28%,for ventilator use it was 24% and for decease status, the predicted probability was 21% respectively. 

The logistic regression and random forest models showed some evidence of overfitting as it was well evident by looking at the data, 
...
The second report feedback suggestions were taken into consideration the race_wb issue was fixed. The socio-economic status variable was not available for us to add it to the analyses. Comments were added in the rmd file at the appropriate places and the sections were labelled properly.
